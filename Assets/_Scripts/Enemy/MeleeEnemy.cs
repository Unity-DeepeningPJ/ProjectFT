...
public class MeleeEnemy : BaseState
{
    ...
    void Update()
    {
        ...
        Debug.Log("Player In Range: " + playerInRange); // playerInRange 값 확인
        Debug.Log("Is Attacking: " + isAttacking); // isAttacking 값 확인
        Debug.Log("Is Idle: " + isIdle); // isIdle 값 확인

        if (isIdle)
        {
            Debug.Log("Idle Time: " + (Time.time - stopTime)); // 멈춰있는 시간 확인
            if (Time.time - stopTime >= idleTime)
            {
                isIdle = false;
                Debug.Log("Moving after Idle"); // Move 호출 확인
        ...
        else if (!isAttacking)
        {
            ...
            {
                Move(); // 기본 이동
                Debug.Log("Moving"); // Move 함수 호출 확인
            }
        }
        else
        {
    ...
    void Move()
    {
        ...
        Debug.Log("Current Velocity: " + rb.velocity); // 현재 속도 확인
    }
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (isAttacking && ((1 << collision.gameObject.layer) & playerLayer) != 0)
        {
            Debug.Log("Attack!");
            // 데미지를 주는 로직 구현
            // 예: collision.gameObject.GetComponent<PlayerHealth>().TakeDamage(attackDamage);
        }
    ...
    void FindPlayer()
    {
        ...
        if (player != null)
        {
            ...
            Debug.Log("Player Found: " + player.name); // 플레이어 찾음 확인
            Debug.Log("Player Transform: " + playerTransform); // 플레이어 Transform 확인
        }
        else
        {
            ...
            isChasing = false;
            //rb.velocity = Vector2.zero; // 플레이어를 찾지 못하면 속도 초기화 (이 부분을 제거)
            Debug.Log("Player Not Found"); // 플레이어 찾지 못함 확인
        }
    }
        ...
        foreach (var go in goArray)
        {
            if ((layer.value & (1 << go.layer)) != 0)
            {
                ...
                if (curDistance < distance && curDistance <= range * range) // 범위 내에 있는지 확인
                {
                    ...
                    Debug.Log("Found object in layer: " + go.name + ", distance: " + curDistance); // 레이어에 속한 오브젝트 찾음 확인
                }
            }
        }
        return closest;
